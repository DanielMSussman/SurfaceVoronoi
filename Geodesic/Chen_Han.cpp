...
namespace Geodesic
{
	...
	bool CChen_Han::UpdateTreeDepthBackWithChoice()
	{
		...
		if (m_QueueForWindows.empty())
		{
			if (!m_QueueForPseudoSources.empty())
			{
				...
				m_depthOfResultingTree = std::max(m_depthOfResultingTree,
	...
	void CChen_Han::ComputeChildrenOfPseudoSourceFromWindow(int indexOfParentVertex)
	{
		...
		double y2 = 0;
		x1 -= model.Edge(indexOfParentOfParent).coordOfOppositeVert(0);
		y1 -= model.Edge(indexOfParentOfParent).coordOfOppositeVert(1);
		x2 -= model.Edge(indexOfParentOfParent).coordOfOppositeVert(0);
		y2 -= model.Edge(indexOfParentOfParent).coordOfOppositeVert(1);
